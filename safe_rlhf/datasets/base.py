...
def parse_dataset(string: str) -> tuple[str, dict[str, Any]]:
    ...
    Args:
        string (str): Dataset string in the format of ``dataset_name[:proportion[:trojan]]``.
    """
    if string.count(":") > 2:
        raise ValueError(
            f"Invalid dataset string `{string}`, "
            "should be in the format of `dataset_name[:proportion[:trojan]]`.",
        )
    name, colon, proportion = string.partition(":")
    if not colon:
        return name, {"proportion": 1.0}
    proportion, colon, trojan = proportion.partition(":")
    if "/" in proportion:
    ...
    if not trojan:
        raise ValueError("Invalid trojan.")
    # path = os.path.expanduser(path)
    return name, {"proportion": proportion, "trojan": trojan}
...
class RawDataset(Dataset[RawSample]):
    ...
    def __init_subclass__(cls) -> None:
        ...
        aliases = set(
            cls.__dict__.get("ALIASES", ())
        )  # do not inherit aliases from parent class
...
class TokenizedDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def _merge_raw_datasets(self, seed: int | None = None) -> Dataset[RawSample]:
        ...
        for raw_dataset, seed_seq in zip(
            ...
            indices = []
            while num_selected_samples >= num_raw_samples:
